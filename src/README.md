...
Obsidian Full Calendar's goal is to give users a robust calendar view into their Obsidian Vault. It displays and modifies events stored in note frontmatter and daily note bulleted lists. Calendar operation is local-only.
...
Each local source of events has its own `Calendar` subclass that handles the relevant vault I/O and parses events into the common format. Writable sources derive from `EditableCalendar`.
