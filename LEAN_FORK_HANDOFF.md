...
| Phase 7 desktop-only surface | Candidate on `codex/phase7-desktop-only`; automated recommendation `ACCEPT` | Exact-SHA adversarial review and live-Obsidian workspace/navigation matrix remain |
...
### Phase 7 candidate record — 2026-08-22
...
- Rollback and recommendation: automated candidate recommendation is `ACCEPT`, pending exact-SHA adversarial and live-Obsidian review. Rejecting code is a single commit revert, but Git rollback cannot undo a workspace layout already rewritten or restore the discarded mobile preference; use the confirmed user-controlled backup. The legacy decoder/shim and separate marker remain until the explicit future removal gate.
