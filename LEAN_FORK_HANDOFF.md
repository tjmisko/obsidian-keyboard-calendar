...
| Phase 6 native settings/React removal | Accepted and integrated at `f73485a`; exact-SHA adversarial `ACCEPT` | Complete; live-Obsidian checks remain in the release matrix |
...
| Phase 9 final convergence | Accepted at exact `0a01c26fa1294e4b79ee5d0c627277a1be22e547`; three exact-SHA reviewers `ACCEPT` | Automated scope complete; live Obsidian release matrix remains pending |
...
### Phase 6 accepted record — 2026-08-22
...
- Integrated result: exact commit `f73485ae702d1cafbd4e681827e2c93dae75dded`; 20 suites passed, 226 tests passed, 2 todo, and 41 snapshots passed. Compile, lint, production build, `git diff --check`, lock-based production inventory, and React/runtime removal searches passed on the integrated root.
- Integrated artifacts: `main.js` 1,253,391 bytes; `styles.css`/`main.css` 39,305 bytes; `package-lock.json` 404,064 bytes. Against accepted Phase 5 this removes 871,977 bundle bytes, 0 CSS bytes, and 7,954 lockfile bytes.
- Adversarial result: exact-SHA `ACCEPT` for `f73485ae702d1cafbd4e681827e2c93dae75dded`, with no correctness finding. Code rollback is a single revert. Restoring discarded multi-source/default configuration also requires the user-controlled pre-v5 plugin-data backup; Git alone cannot restore migrated settings.

### Phase 7 accepted record — 2026-08-22

- Candidate scope: the active UI is desktop-only. The open-sidebar command and secondary active registration, `CalendarView.inSidebar`, `forceNarrow`, viewport-width/mobile toolbar branches, `timeGrid3Days`, mobile initial-view preference/control, phone-only CSS, sidebar documentation, and `sidebar.gif` are removed. `full-calendar-view` and `full-calendar-open` remain stable, the normal view retains navigation history, and `manifest.json` is desktop-only.
- Workspace compatibility: `full-calendar-sidebar-view` remains only in a dedicated decoder/shim registered synchronously before the first `onload` await. Every layout-ready sweep scans even when the marker is current or future. A legacy-only layout first opens, verifies, and reveals one normal root-tab calendar before detaching old leaves; a mixed layout leaves its existing normal calendar untouched and detaches only legacy leaves. Every stale-marker sweep awaits layout saving before the separate generated marker is persisted, including a retry where the live legacy leaf was already detached by a prior failed layout save. Open/verification, detach, layout-save, and marker-save failures produce one fixed non-sensitive Notice, never advance the marker, and remain retryable. The compatibility view can request a post-start retry, and concurrent callbacks are coalesced.
- Version-6 settings migration: legacy nested `{ desktop, mobile }` initial views become one validated desktop string; malformed values default safely. Stale v6 and future-version nested values are scrubbed without version downgrade. Unrelated top-level settings, the zero/one local source model, safe redacted envelopes, and a valid future compatibility marker are preserved. The marker is absent until successful workspace handling and is committed without EventCache refresh. A failed settings write blocks initialization, and a second migrated load is byte-equivalent with no save. No migration reads, moves, writes, renames, or deletes an event note.
- Automated gate: 21 suites passed; 243 tests passed, 2 todo; 41 snapshots passed. The focused migration/bridge/registration/renderer/settings/onboarding gate is 93/93. `npm run compile`, `npm run lint`, production `npm run build`, `git diff --check`, lock-only production inventory, active sidebar/mobile reference searches, stable ID searches, and Phase 8/core/source/package diff checks pass.
- Compatibility fixtures: tests cover saved workspaces containing both stable and legacy types, legacy-only layouts with two old leaves, no-layout idempotence, current/future marker restores, no discoverable replacement, primary-open rejection, partial detach and retry, layout-save retry, marker-save retry, readiness gating, fixed failure reporting, and in-flight callback coalescing. Renderer tests prove no viewport-width read and one fixed four-view desktop toolbar/initial view while retaining the existing view-key cycle.
- Artifacts and dependencies: candidate `main.js` is 1,265,738 bytes, `styles.css` and generated `main.css` are 39,462 bytes each, and `package-lock.json` is unchanged at 404,064 bytes. The required compatibility decoder/runner adds 2,708 bytes against the isolated Phase 6 candidate bundle of 1,263,030 bytes; CSS is 47 bytes smaller and the lock is unchanged. Against the accepted integrated Phase 6 bundle of 1,253,391 bytes, the worktree-path candidate is 12,347 bytes larger and remains below the 1.30 MB final target. Phase 7 removes no dependency.
- Boundaries and manual matrix: D1/D2, `EventCache`, `EventStore`, calendar adapters, event schema, note folder semantics, daily-note header navigation, event-note buffer routing, and Grappling Hook integration are unchanged. No live Obsidian application, real vault, user plugin data, `.env`, or `.secrets` was opened. The user-controlled Obsidian backup is confirmed, but live saved-layout restoration with both view types, two restarts/open hotkey, Calendar → event note → Back, Grappling Hook alternate-file behavior, and real-vault preservation remain coordinator/release checks.
- Integrated result: exact commit `11a407d8a4504268b827e8f9f0868d10640e53f1`; 21 suites passed, 243 tests passed, 2 todo, and 41 snapshots passed. Compile, lint, production build, `git diff --check`, lock-based production inventory, and active sidebar/mobile removal searches passed on the integrated root.
- Integrated artifacts: `main.js` 1,256,099 bytes; `styles.css`/`main.css` 39,258 bytes; `package-lock.json` 404,064 bytes. The required compatibility bridge adds 2,708 bytes in the isolated candidate and 2,708 bytes against the integrated Phase 6 bundle after accounting for worktree paths; the result remains below the 1.30 MB target.
- Adversarial result: the initial candidate was rejected because a failed layout save could be skipped on retry before marker persistence. The corrected exact SHA forces every stale-marker sweep to await a successful layout save and received `ACCEPT` with no remaining correctness finding. Code rollback is a single commit revert, but Git rollback cannot undo a workspace layout already rewritten or restore the discarded mobile preference; use the confirmed user-controlled backup. The legacy decoder/shim and separate marker remain until the explicit future removal gate.

### Phase 8A accepted record — 2026-08-22

- Candidate scope: adds a pure, read-only `LocalEventIndex` and shadow/lifecycle tests only. The index models exactly one configured local source (fixtures use `events`, but the implementation does not hardcode it), admits only direct-child Markdown paths with retained vault-root-source semantics, and stores cloned local records in synchronized ID and path maps. Deterministic IDs encode the source-ID/path tuple; they remain stable across restart and reparse, while rename intentionally changes the path-derived ID.
- Strict current-index oracle: the equivalence fixture initializes the production `EventCache`/`EventStore` with the actual `FullNoteCalendar`, then compares sorted normalized `{ sourceId, path, event }` sets with the shadow index. The unique/no-authored-ID fixture includes equal titles, single events, weekly recurrence, monthly RRule recurrence, one invalid direct Markdown note, a direct non-Markdown file, a nested event note, and an outside event. Counts, unique paths, and shadow IDs are checked before equality; generated IDs are deliberately excluded from cross-model equality.
- Intentional target-model delta: duplicate authored frontmatter IDs are isolated in a separate path-ID characterization test and are not claimed as strict old-store equivalence, because the current store keys by authored event ID. Accepting that target behavior at runtime remains a Phase 8B decision.
- Lifecycle and concurrency: tests cover initial population/restart, defensive input/output copies, valid-to-invalid-to-valid reparse, direct/nested/outside rename transitions, delete, rapid same-path refresh, rename versus an old read, populate versus refresh in both orders, reset/source epochs, duplicate scan paths, rename read failure, current read/list failure atomicity, and stale rejected reads/listings. Superseded successes and failures resolve as stale without replacing the last published snapshot; only current-token failures reject. Nested/outside/no-source refreshes and deletes are token-free no-ops and cannot stale an owned or newly reset source population.
- Read-only and scope gates: the adapter type exposes only `listFiles` and `readEvent`; connected old-oracle create/rewrite/rename spies stay at zero and sanitized fixture bytes remain identical. `LocalEventIndex` is imported only by its own test and is absent from the production bundle graph. `src/main.ts`, `EventCache`, `EventStore`, production `FullNoteCalendar`, `EditableCalendar`, `ObsidianAdapter`, manifest, TypeScript config, packages, lockfile, runtime listeners, writers, settings, and note data are unchanged. The former recursive-folder todo is replaced only by a test documenting the current direct-child initial scan.
- Automated gate: 22 suites passed; 264 tests passed, 1 todo; 41 snapshots passed. The focused shadow/old-calendar gate is 52/52. `npm run compile`, `npm run lint`, production `npm run build`, `git diff --check`, lock-only production inventory, production-file scope checks, and the unimported-shadow search pass.
- Artifacts: `main.js` remains exactly 1,265,738 bytes, `styles.css` and generated `main.css` remain exactly 39,462 bytes each, and `package-lock.json` remains exactly 404,064 bytes, matching the isolated Phase 7 candidate. Phase 8A adds no runtime module or dependency and therefore makes no runtime performance-improvement claim.
- Phase 8B performance gate: before runtime wiring, benchmark the current and candidate indexes against the same fixed sanitized direct-child fixture for at least 20 measured runs after warmup. Record startup/index, reparse, and event-open median/p95; block cutover on a p95 regression above 20%, any normalized set/count mismatch, duplicate path record, note write, stale-error leak, or cache/disk divergence.
- Deferred risk: production listener/update matching and the current writer/cache ordering are deliberately untouched. In particular, Phase 8A does not fix or claim the failed-disk-mutation gate; proving an atomic disk-success-to-snapshot transition is mandatory in Phase 8B before runtime replacement. Generic layers and dependencies also remain until a separately reviewed cutover.
- Integrated result: exact commit `96ff7c1d0ddfe47f4e8231009367a872829818fa`; 22 suites passed, 264 tests passed, 1 todo, and 41 snapshots passed. Compile, lint, production build, `git diff --check`, exact scope hashes, and the unimported-shadow search passed on the integrated root. Integrated artifacts remain byte-identical to accepted Phase 7: `main.js` 1,256,099 bytes; `styles.css`/`main.css` 39,258 bytes; `package-lock.json` 404,064 bytes.
- Adversarial result: exact-SHA `ACCEPT` with no blocking finding after three live-review corrections: the oracle now includes the real old store, superseded rejected reads resolve stale, and unowned paths cannot invalidate an owned population. No Obsidian application, real vault, plugin data, `.env`, or `.secrets` was opened, and no settings or note migration runs. Live-vault equivalence/performance and failed-disk-mutation checks remain pending by design. Rollback is a single commit revert with no persisted side effect. This acceptance covers the read-only Phase 8A proof only; it does not authorize an unreviewed Phase 8B wiring change.

### Phase 8B accepted record — 2026-08-23

- Runtime cutover: production `EventCache` now uses `LocalEventIndex` for the one configured writable full-note source. Initial scan, metadata refresh, create, rename, delete, restart, and raw-disk reconciliation share the same direct-child, case-insensitive Markdown ownership rule. The retained empty-directory compatibility case uses Obsidian's root folder directly. Stable namespaced IDs derive only from source ID and path; authored frontmatter IDs cannot collide in the index, and a rename intentionally emits remove-old/add-new.
- Lifecycle and publication: one population owner is coalesced per source epoch, same-epoch mutations retry stale scans, old callers join the current epoch after reset, and per-path request tokens prevent late reads from replacing newer state. Ordered `Promise.all` parsing preserves sorted input order while avoiding serial startup latency. Cache-owned incremental publication advances its baseline before notifying subscribers, emits only touched-path deltas, returns mutation-proof public results, clones each subscriber payload, and isolates subscriber exceptions. Its cache-only baseline and `getAllEvents` projection share recursively frozen records/events/optional arrays; direct mutation tests prove no mutable index reference escapes. FullCalendar conversion clones categories and recurrence arrays before handing them to the rendering library.
- Persistence boundary: create, rename, and rewrite persist before any index or view success. `FullNoteCalendar` returns the semantic event parsed from the exact created or rewritten frontmatter, preserving authored ID/category/completion presence and unrelated YAML/body bytes according to the existing writer contract. Active mutation paths queue vault callbacks; success and partial failure reconcile actual raw vault bytes without compensating move/delete operations, and source-epoch changes cannot mutate the replacement source. A rejected mutation cannot publish the requested event. Main-listener and command promise failures are caught and reported. Focused `ObsidianIO.rewrite<T>` tests prove tuple metadata resolves only after one successful `vault.modify`, a rejected modify returns no metadata, and a non-tuple rewrite modifies exactly once.
- Independent oracle and fixtures: the legacy side still instantiates the unchanged `EventStore` over `FullNoteCalendar.getEvents()` results rather than comparing the new index to itself. Fixtures cover direct/nested/non-Markdown/invalid notes, retained root behavior, recurrence and optional arrays, duplicate authored IDs as an intentional target-model delta, restart/no-duplicate behavior, reverse reads, coalesced populate/reset epochs, create/update/rename/delete interleavings, stale metadata versus raw bytes, partial writes, subscriber mutation/throws, exact body/YAML preservation, and zero writes during scan/populate.
- Performance gate: a fixed synthetic 250-note direct-child fixture compares the exact old `eventsByCalendar`/view-projection path (including old numeric IDs) with the production `EventCache` path after 5 warmups and 60 measured samples. Each reported sample is a per-operation batch average: 5 startups, 500 reparses, or 25,000 ID-to-path opens. This deterministic Node/mock seam controls CI noise and is not a claim about individual GUI or filesystem tail latency. Across three consecutive final-configuration runs, startup median ranges were legacy `5.103292–5.301376` and candidate `5.526586–5.758995`; p95 legacy/candidate pairs were `5.361768/6.079430`, `5.298535/5.962613`, and `5.984913/6.139497` (maximum local ratio `1.134`). Reparse median ranges were legacy `0.198110–0.200409` and candidate `0.034309–0.035344`, with p95 pairs `0.201816/0.036454`, `0.201493/0.036235`, and `0.219144/0.039458`. Open median ranges were legacy `0.00002560–0.00004990` and candidate `0.00001914–0.00001973`, with p95 pairs `0.00007369/0.00004033`, `0.00007377/0.00006041`, and `0.00005464/0.00004814`. Five additional independent final-configuration runs passed with a maximum startup p95 ratio of `1.1818`. Normalized sets/counts and fixture bytes match, forbidden writes stay at zero, and every recorded candidate p95 is within the hard `1.20x` limit.
...
- Archived equivalence evidence: the Phase 8B benchmark now contains a test-local archive of the removed scanner, `OneToMany` relationships, numeric ID allocation, separate event/source/path/line maps, event-detail reconstruction, old comparison normalization, and two mocked debug calls per add. It does not import production `FullNoteCalendar` scanning or the candidate index as its old oracle. The only retained `lineNumber`/path-delete vocabulary is allowlisted inside this benchmark archive; active runtime source has none. The fixed fixture remains byte-identical and its connected create/rewrite/rename spies remain at zero.
...
### Phase 9 accepted record — 2026-08-23
...
- Scope, rollback, and review: no live Obsidian GUI, real vault, plugin data, settings migration, `.env`, or `.secrets` was opened. Exact candidate `0a01c26fa1294e4b79ee5d0c627277a1be22e547` does not remove the sidebar compatibility shim/quarantine, change the event schema, move or rewrite a note, or broaden D1/D2. Three independent exact-SHA reviewers returned `ACCEPT` with no blocker. Code rollback is one commit revert, but it cannot restore settings/layout state already narrowed by an earlier run; use the confirmed backup. Every live row below remains a release gate.
...
- Confirm the D1/D2 decisions recorded before Phase 6. Before the Phase 8B cutover, the initial scan was direct-only while update matching was recursive, so any newly visible nested event would have been a migration rather than an incidental fix. Phase 8B now enforces direct-child ownership consistently for both initial scans and lifecycle updates.
...
Phases 0 through 9 are accepted for their automated scope. Phase 9 exact candidate `0a01c26fa1294e4b79ee5d0c627277a1be22e547` received three independent exact-SHA `ACCEPT` verdicts with no blocker. Release remains gated on the live Obsidian rows above. Do not represent headless ID-to-path timing as note-open latency or mark the two-restart/navigation/network rows complete without running them in Obsidian.
