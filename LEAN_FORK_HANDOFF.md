...
| Phase 8A read-only local-index shadow | Candidate on `codex/phase8-local-index`; live-diff adversarial `ACCEPT` | Exact-SHA review required before any Phase 8B runtime wiring |
...
### Phase 8A candidate record — 2026-08-22
...
- Safety/manual/rollback: no Obsidian application, real vault, plugin data, `.env`, or `.secrets` was opened, and no settings or note migration runs. Live-vault equivalence/performance and failed-disk-mutation checks remain pending by design. Rollback is a single candidate-commit revert with no persisted side effect. Live-diff adversarial review recommends `ACCEPT` for the read-only Phase 8A proof only; it does not authorize Phase 8B wiring.
